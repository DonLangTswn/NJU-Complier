...
Operand alloc_VAR(char* id) {
    ...
    if (table_VAR[symInd].vtype == _UNDEF) {
        table_VAR[symInd].vtype = _VAR;
        table_VAR[symInd].prefix = _NOTHING;
        table_VAR[symInd].value.vint = cnt_VAR;
        table_VAR[symInd].offset = cnt_offset += getSizeOf(getSymb(id)->dType);   // add in lab-4
        cnt_VAR++;
    }
    return table_VAR[symInd];
}
Operand alloc_TEMP() {
    Operand tmp = newTEMP(cnt_TEMP++);
    tmp.offset = cnt_offset += 4;   // add in lab-4
    DIY_DEBUG(tmp.offset, %d)
    return tmp;
}
Operand alloc_LABEL() {
...
void transl_ParamDec(LevelNode* ParamDec, unsigned int param) {
    LevelNode* VarDec = ParamDec->childhead->next;
    transl_VarDec(VarDec, param);
}

// `VarList -> ParamDec , VarList | ParamDec`
void transl_VarList(LevelNode* VarList, unsigned int param) {
    LevelNode* ParamDec = VarList->childhead;
    transl_ParamDec(ParamDec, param);
    if (ParamDec->next) {
        transl_VarList(ParamDec->next->next, param + 1);
    }
}
...
Operand transl_VarDec(LevelNode* VarDec, unsigned int param) {
    if (!strcmp(VarDec->childhead->msg, "VarDec")) {    // VarDec [ INT ]
        VarDec = VarDec->childhead;
        return transl_VarDec(VarDec, param);  // 放心交给`ID`去处理
    }
    else {  // ID
        ...
        if (param > 0) {  // 函数参数
            ...
            VarIsParam[oprd.value.vint] = param; // 函数参数, 表示第几个参数
        }
...
int inter_repres(LevelNode* root, bool ir_out) {
    initEntryIRNode();
    initVarFuncTable();
