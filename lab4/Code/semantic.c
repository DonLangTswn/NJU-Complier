...
// 将新节点加入变量符号表
void addSymbTable(SymbNode* node) {
...
// 将新节点加入函数符号表
void addFuncTable(FuncNode* node) {
...
void initIndexedTable() {
    // ---------- ADDED IN LAB-3 ----------
    FuncNode* read = newFuncNode("read");
    read->def = 1;
    read->rType.type = _INT;
    addFuncTable(read);

    FuncNode* write = newFuncNode("write");
    write->def = 1;
    write->rType.type = _INT;
    write->rType.value.Int = 0;
    SymbNode* w_param = newSymbNode("");
    w_param->def = 1;
    w_param->dType.type = _INT;
    write->params = w_param;
    addFuncTable(write);
    // ---------- ADDED IN LAB-3 ----------
    // 初始化变量名表
...
ExpType handle_Exp(LevelNode* Exp) {
    ...
    else if (!strncmp(opr->msg, "RELOP", 5)) {
