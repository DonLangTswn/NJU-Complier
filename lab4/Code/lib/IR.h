...
Operand *table_VAR;     // 记录`符号表下标 - VAR_NO`的映射, `VAR_NO > 0`
...
void initVarFuncTable() {
    table_VAR  = (Operand *)malloc(sizeof(Operand) * symbTableSize);
    table_FUNC = (int *)malloc(sizeof(int) * funcTableSize);
    VarIsParam = (int *)malloc(sizeof(int) * symbTableSize);
    for (int i = 0; i < symbTableSize; i++) {
        table_VAR[i].vtype = _UNDEF;
    }
...
bool isparam(Operand var) {
    return var.vtype == _VAR && VarIsParam[var.value.vint] > 0;
}
...
Operand transl_VarDec(LevelNode *, unsigned int);
...
void transl_VarList(LevelNode *, unsigned int);
void transl_ParamDec(LevelNode *, unsigned int);
