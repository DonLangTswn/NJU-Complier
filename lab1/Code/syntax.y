...
%type<Node> VarDec FuncDec VarList ParamDec
...
ExtDef:  Specifier ExtDecList SEMI { // 全局变量定义：int g1, g2;
    ...
    | Specifier FuncDec CompSt {     // 函数定义：int/ main()/ {...}/
        struct levelNode* newNode = createNode("ExtDef", $1->line, "NON_TERMINAL");
        addChild(newNode, $1);
        addChild(newNode, $2);
        addChild(newNode, $3);
...
VarDec:  ID {            // a
    ...
    ;
FuncDec:  ID LP VarList RP { // func(int x, float y[3],...), 内为VarList
